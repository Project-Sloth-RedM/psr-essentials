...
local function updatePermissionLevel()
    PSRCore.Functions.TriggerCallback('qb-afkkick:server:GetPermissions', function(userGroups)
...
RegisterNetEvent('PSRCore:Client:OnPermissionUpdate', function()
    updatePermissionLevel()
end)
                        ...
                        if time then
                            if time > 0 then
                                ...
                                if _type == 'minutes' then
                                    PSRCore.Functions.Notify('You are AFK and will be kicked in ' .. math.ceil(time / 60) .. ' minute(s)!', 'error', 10000)
                                elseif _type == 'seconds' then
                                    PSRCore.Functions.Notify('You are AFK and will be kicked in ' .. time .. ' seconds!', 'error', 10000)
                                end
                                time -= 1
                            else
